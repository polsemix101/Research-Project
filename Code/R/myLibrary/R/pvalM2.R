...
pvalM2 <- function(TSx, TSy, emb, ent, beta){
  ...
  switch(ent,
         ...
         "S" = {
           ...
           mu_x <- HShannon(q_x)
           sig2_x <- sigma2qM2(TS = TSx, emb = emb, ent = "S")

           mu_y <- HShannon(q_y)
           sig2_y <- sigma2qM2(TS = TSy, emb = emb, ent = "S")
         ...
         "T" = {
           ...
           sig2_x <- sigma2qM2(TS = TSx, emb = emb, ent = "T", beta = beta)

           mu_y <- HTsallis(q_y, beta = beta)
           sig2_y <- sigma2qM2(TS = TSy, emb = emb, ent = "T", beta = beta)
         ...
         "R" = {
           ...
           mu_x <- sum(q_x^beta)
           sig2_x <- sigma2qM2(TS = TSx, emb = emb, ent = "R", beta = beta)

           mu_y <- sum(q_y^beta)
           sig2_y <- sigma2qM2(TS = TSy, emb = emb, ent = "R", beta = beta)
         ...
         "F" = {
           ...
           mu_x <- HFisher(q_x)
           sig2_x <- sigma2qM2(TS = TSx, emb = emb, ent = "F")

           mu_y <- HFisher(q_y)
           sig2_y <- sigma2qM2(TS = TSy, emb = emb, ent = "F")
